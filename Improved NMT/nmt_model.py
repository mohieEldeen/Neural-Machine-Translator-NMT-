...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_padded = self.vocab.tgt.to_input_tensor( target , self.device ) # (max_sentence_length, batch_size)
        source_padded_chars = self.vocab.src.to_input_tensor_char( source , self.device ) # (max_sentence_length, batch_size, max_word_length)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char( target , self.device ) # (max_sentence_length, batch_size, max_word_length)

        enc_hiddens, dec_init_state = self.encode(source_padded_chars, source_lengths)

        enc_masks = self.generate_sent_masks(enc_hiddens, source_lengths)
        combined_outputs = self.decode(enc_hiddens, enc_masks, dec_init_state, target_padded_chars) # (max_sentence_length, batch_size, hidden_size)



        ### END YOUR CODE

        P = F.log_softmax(self.target_vocab_projection(combined_outputs), dim=-1) # (max_sentence_length, batch_size, target_vocab_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        src_embeddings = self.model_embeddings_source(source_padded) # (seq_len , batch , embedding_size) after the embedding


        packed_embeddings = pack_padded_sequence(src_embeddings , source_lengths) #packing the padded embeddings (the outpus is the batch data and the batch_sizes)

        (packed_hidden_states_all , (last_hidden_states , last_cell_states)) = self.encoder(packed_embeddings)  #the output contains the packed hidden_states over time and the last cell and hidden states for every direction and layer

        enc_hiddens = pad_packed_sequence(packed_hidden_states_all)[0].permute(1,0,2) #shape (batch , seq_len , 2* hidden_size) from two directions


        #to get the intial states of the decoder we only need the final states of the last layer only in both directions so we discard the first layer
        last_hidden_states = torch.cat([last_hidden_states[0], last_hidden_states[1]] , dim = 1) #reshaping the output to be (batch_size , 2* hidden_size)
        last_cell_states = torch.cat([last_cell_states[0], last_cell_states[1]] , dim = 1) #reshaping the output to be (batch_size , 2* hidden_size)


        #the decoder is unidirectional so its intial state are both of shape (batch , hidden_size)
        decoder_h0 = self.h_projection(last_hidden_states) #shape (batch , hidden_size)
        decoder_c0 = self.c_projection(last_cell_states) #shape (batch , hidden_size)
        dec_init_state = (decoder_h0 , decoder_c0)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hidden_proj = self.att_projection(enc_hiddens)  #(batch , seq_len , h)


        tgt_embeddings = self.model_embeddings_target(target_padded) # (seq_len-1 , batch , embedding_size) after the embedding


        for tgt_padded_t in torch.split(tgt_embeddings , 1, dim=0):

            y_t = torch.squeeze(tgt_padded_t, dim=0) #shape (batch , embedding_size)

            y_combined_t = torch.cat([y_t.contiguous() , o_prev.contiguous()] , dim = 1)  #shape (batch , embedding_size+hidden_size)


            dec_state , o_t , _ = self.step (y_combined_t , dec_state , enc_hiddens , enc_hidden_proj  , enc_masks) # o_prev of shape (batch , h)
        ...
        combined_outputs = torch.stack(combined_outputs ,dim = 0) #shape (seq_len-1 , batch , hidden_size)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decoder(Ybar_t , dec_state) #applying the decoder LSTM and getting the tuple (h_t , c_t) both of shape (batch_size , hidden_size)
        (dec_hidden, dec_cell) = dec_state #shape (batch_size , hidden_size )


        #the matrix mul between the projected hidden states of the encoder and the output hidden_state of the decoder to measure the similarity
        # (batch_size , seq_len , hidden_size) x (batch_size , hidden_size , 1)
        e_t = torch.bmm(enc_hiddens_proj , dec_hidden.unsqueeze(2)) #shape (batch_size , seq_len ,1)
        e_t = torch.squeeze(e_t , dim = 2) #shape (batch_size , seq_len )
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        alpha_t = torch.nn.functional.softmax(e_t  , dim = 1) #converting the similarity into probability destribution on the sentence dimension (batch_size , seq_len )

        #multiplying the probabilities to its coresponding vectors of the hidden states of the encoder then sum over the sentence dim to have our weighted sum
        #we can easily do that using the batch mm
        #             (batch_size,1, seq_len) x (batch_size , seq_len , 2*hidden_size) >> (batch_size , 1 , 2* hidden_size)
        a_t = torch.bmm(alpha_t.unsqueeze(1) , enc_hiddens)  #shape (batch_size ,1, 2* hidden_size)

        a_t = a_t.squeeze(1) #shape (batch_size , 2* hidden_size)

        u_t = torch.cat([a_t , dec_hidden] , dim = 1) #stacking the weighted sum with the current hidden state of the decoder (batch , 3*h)

        v_t = self.combined_output_projection(u_t) #(batch , h)

        o_t = self.dropout(torch.tanh(v_t))  #(batch , h)
