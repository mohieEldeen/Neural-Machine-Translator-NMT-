...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_char2id = self.words2charindices(sents) #list[list[list[int]]]
        sents_chars_padded = pad_sents_char(sents_char2id, self.char_pad) #list[list[list[int]]] (batch , max_seq_len , max_word_len)

        sents_chars_padded = torch.tensor(sents_chars_padded, dtype=torch.long, device=device).permute(1,0,2) #tensor of shape (max_sentence_length, batch_size, max_word_length)


        return  sents_chars_padded
